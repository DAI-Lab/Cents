...
class Generator(nn.Module):
    def __init__(self, noise_dim, embedding_dim, final_window_length, input_dim):
        ...
        self.final_window_length = final_window_length // 8  # Ensure the final window length is correctly calculated
        self.input_dim = input_dim

        self.month_embedding = nn.Embedding(12, embedding_dim)
        self.day_embedding = nn.Embedding(7, embedding_dim)

        self.fc = nn.Linear(noise_dim + 2 * embedding_dim, self.final_window_length * 64)
...
class ACGAN:
    def __init__(self, input_dim, noise_dim, embedding_dim, output_dim, learning_rate, weight_path):
        self.code_size = noise_dim
        self.batch_size = 256
        self.learning_rate = learning_rate
        self.weight_path = weight_path

        assert output_dim % 8 == 0, "output_dim must be a multiple of 8 in this architecture!"
        final_window_length = output_dim 

        self.generator = Generator(noise_dim, embedding_dim, final_window_length, input_dim).to(device)
        self.discriminator = Discriminator(output_dim).cuda() 
    ...
    def train(self, x_train, x_val, num_epoch=5):
        ...
        for epoch in range(num_epoch):
            for i, (time_series_batch, month_label_batch, day_label_batch) in enumerate(tqdm(train_loader, desc=f"Epoch {epoch + 1}")):
                ...
                current_batch_size = time_series_batch.size(0)  # Get the actual batch size
                noise = torch.randn((current_batch_size, self.code_size)).to(device)
                generated_time_series = self.generator(noise, day_label_batch, month_label_batch)

                soft_zero, soft_one = 0, 0.95

                self.optimizer_D.zero_grad()
                real_pred, real_day, real_month = self.discriminator(time_series_batch)
                fake_pred, fake_day, fake_month = self.discriminator(generated_time_series.detach().squeeze())

                d_real_loss = (
                    self.adversarial_loss(real_pred, torch.ones_like(real_pred) * soft_one)
                    + self.auxiliary_loss(real_day, day_label_batch)
                    + self.auxiliary_loss(real_month, month_label_batch)
                )

                d_fake_loss = (
                    self.adversarial_loss(fake_pred, torch.ones_like(fake_pred) * soft_zero)
                ...
                gen_month_labels = torch.randint(5, 11, (current_batch_size,)).to(device)
                generated_time_series = self.generator(noise, gen_day_labels, gen_month_labels)

                validity, pred_day, pred_month = self.discriminator(generated_time_series.detach().squeeze())

                g_loss = (
                    self.adversarial_loss(validity, torch.ones_like(validity) * soft_one)
            ...
            with torch.no_grad():
                for time_series_batch, month_label_batch, day_label_batch in val_loader:
                    ...
                    mmd = mmd_loss(time_series_batch.cpu().numpy(), x_generated.squeeze())
                    summary_writer.add_scalars(
                        "data/mmd_loss",
                        {"load": mmd},
                        global_step=epoch,
